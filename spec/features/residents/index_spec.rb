  ...
  describe "3. As a Visitor" do
    describe "when I visit /residents" do
      it "I see the name of each resident record in the system" do
        ...
        jones = Resident.create!(family_name: "Jones", long_term_housing_need: true, family_size: 4, disaster_shelters_id: "#{arlington_life.id}")
        bradley = Resident.create!(family_name: "Bradley", long_term_housing_need: false, family_size: 2, disaster_shelters_id: "#{austin_street.id}")
        visit "/residents"
        ...
        expect(page).to have_content(jones.disaster_shelters_id)
        ...
        expect(page).to have_content(bradley.disaster_shelters_id)
